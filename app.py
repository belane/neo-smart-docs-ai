...
with col1:
    query = st.text_input('Question', key='question', label_visibility='hidden', placeholder='Ask any question about Neo blockchain')
with col2:
    st.write('#')
...
if query.strip() or (button and query.strip()):
    with st.spinner('Searching ...'):
        try:
            ...
            references = '- ' + '\n- '.join(set([ref['file_path'] for ref in response.extra_info.values()]))
