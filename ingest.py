...
for doc in docs:
    ...
    if doc.extra_info["file_path"] == "README.md":
        parsed_docs.remove(doc)
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://docs.neo.org/"
        + doc.extra_info["file_path"].replace(".md", ".html").replace("\\", "/")
    ).geturl()
...
for doc in docs:
    ...
    if "tutorials" in doc.extra_info["file_path"]:
        ...
        doc.extra_info["file_path"] = urlparse(
            "https://developers.neo.org/"
            + doc.extra_info["file_path"].replace(".md", "").replace("\\", "/")
        ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
# Add to document library
...
for doc in docs:
    ...
    if doc.extra_info["file_path"] == "nep-X.mediawiki":
        ...
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/neo-project/proposals/blob/master/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
# Add to document library
...
for doc in docs:
    ...
    doc.extra_info["file_path"] = urlparse(
        "https://dojo.coz.io/"
        + doc.extra_info["file_path"]
        .replace("docs\\source\\", "neo3/mamba/")
        .replace(".md", ".html")
        .replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
...
for doc in docs:
    ...
    doc.extra_info["file_path"] = urlparse(
        "https://dojo.coz.io/"
        + doc.extra_info["file_path"]
        .replace("docs\\", "neo3/neon-js/docs/")
        .replace(".md", "")
        .replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
...
for doc in docs:
    if doc.extra_info["file_path"] == "docs\\release-instruction.md":
        ...
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/nspcc-dev/neo-go/blob/master/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
...
doc_urls = ["https://neoline.io/dapi/N3.html"]
docs = TrafilaturaWebReader(True).load_data(doc_urls)
# Parse references
for i, doc in enumerate(docs):
    doc.extra_info = {"file_path": doc_urls[i]}
...
doc_urls = ["https://neon.coz.io/wksdk/core/index.html"]
docs = TrafilaturaWebReader(True).load_data(doc_urls)
# Parse references
for i, doc in enumerate(docs):
    doc.extra_info = {"file_path": doc_urls[i]}
...
for doc in docs:
    if doc.extra_info["file_path"] != "README.md":
        ...
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/CityOfZion/wallet-connect-sdk/blob/main/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
...
for doc in docs:
    if doc.extra_info["file_path"] != "README.md":
        ...
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/neo-project/neo-modules/blob/master/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
...
for doc in docs:
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/N3developertoolkit/neo-worknet/blob/main/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
...
for doc in docs:
    ...
    if "legacy" in doc.extra_info["file_path"]:
        ...
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/neo-project/neo-express/blob/master/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
...
for doc in docs:
    ...
    if "legacy" in doc.extra_info["file_path"]:
        ...
        continue
    doc.extra_info["file_path"] = urlparse(
        "https://github.com/neo-project/neo-debugger/blob/master/"
        + doc.extra_info["file_path"].replace("\\", "/")
    ).geturl()
    # remove 'file_name' for storage optimization
    del doc.extra_info["file_name"]
